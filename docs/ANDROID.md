...
- Если все прямые зеркала недоступны, оболочка однократно включает HTTP-proxy fallback, собранный из локальных `trackAnimeProxy*` в `android/local.properties`, и повторяет цепочку зеркал. Proxy override поддерживается не всеми старыми системными WebView. Данные proxy попадают в APK, поэтому proxy должен ограничивать назначения whitelist-ом и не давать доступ к другим доменам.
