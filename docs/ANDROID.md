...
- Если все прямые зеркала недоступны, оболочка однократно включает выбранный HTTP-proxy fallback и повторяет цепочку зеркал. В «Настройках приложения» доступны режимы: без прокси, proxy из `trackAnimeProxy*` в `android/local.properties` и ручной адрес/порт с опциональными логином и паролем. Ручные параметры хранятся локально в данных приложения и не передаются сайту. Proxy override поддерживается не всеми старыми системными WebView; proxy должен ограничивать назначения whitelist-ом и не давать доступ к другим доменам.
