...
- Если все прямые зеркала недоступны, оболочка однократно включает выбранный HTTP-proxy fallback и повторяет цепочку зеркал. В «Настройках приложения» доступны режимы: без прокси, proxy из `trackAnimeProxy*` в `android/local.properties` и ручной адрес/порт с опциональными логином и паролем; также выбор скрытия системных панелей Android — показывать, всегда скрывать (immersive) или только в полноэкранном видео (режим по умолчанию, как раньше). Ручные параметры прокси и режим панелей хранятся локально в данных приложения и не передаются сайту. Proxy override поддерживается не всеми старыми системными WebView; proxy должен ограничивать назначения whitelist-ом и не давать доступ к другим доменам. Внутри Android-оболочки пункт «Настройки приложения» также есть в выпадающем меню профиля (deep link `trackanime://settings`); вне оболочки пункт скрыт.
