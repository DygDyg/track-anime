...
async function buildVkNotificationDelivery(payload: HistoryNewNotificationPayload): Promise<{
  message: string;
...
export async function sendVkNotification(
  ...
  const { message, keyboard } = await buildVkNotificationDelivery(payload);
