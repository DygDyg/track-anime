...
const MOBILE_AVATAR_RING_SRC = "/avatar-ring.apng";
...
export function BottomNavProfileFab() {
  ...
  const avatarSrc = user ? shikimoriAvatarUrlLarge(user.avatar) : null;
  ...
  const fabButtonClass = [
    "pwa-bottom-nav-fab inline-flex h-full w-full items-center justify-center overflow-hidden rounded-full",
      ...
      <div className="pwa-bottom-nav-fab-slot">
        ...
        >
          {avatarSrc ? (
            ...
            <span className="flex h-full w-full items-center justify-center bg-accent/15 text-lg font-semibold text-accent">
        ...
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={MOBILE_AVATAR_RING_SRC}
          alt=""
          aria-hidden
          className="pwa-bottom-nav-fab-ring"
          decoding="async"
        />
      </div>
