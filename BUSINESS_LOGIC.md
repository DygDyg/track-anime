...
11. Beta-плеер может создать WebSocket-комнату совместного просмотра, если глобальная настройка `WatchPartySettings.enabled` включена. В комнате есть invite-ссылка, копируемый пятизначный цифровой ключ комнаты для ручного подключения, список участников и мастер. Если пользователь вводит ключ комнаты другого тайтла, клиент переходит на страницу этого тайтла и повторно подключается по тому же ключу. Неавторизованные пользователи входят со случайным локальным именем вида «Гость 1234», если `WatchPartySettings.allowGuests` включена. Мастер управляет play/pause, seek, сериями и озвучкой; play/pause, перемотка, выбор серий и смена озвучек для остальных включаются отдельными разрешениями мастера. Позиция комнаты синхронизируется напрямую в секундах (`positionSeconds`) как seek API Kodik, а мастер отправляет полное состояние комнаты каждые 2 секунды, включая паузу. В комнате источником истины для сезона/серии/озвучки и состояния play/pause является состояние TA-интерфейса комнаты, а Kodik остаётся источником текущих секунд просмотра. События `current_episode` от iframe Kodik не меняют TA-серию; watchdog периодически сверяет iframe с TA-состоянием и при рассинхроне повторно запускает play/pause или пересоздаёт iframe на нужной серии. В комнате iframe по возможности монтируется по `KodikEpisode.playerLink` конкретной серии, чтобы общий playerLink не подхватывал локальное сохранённое «Продолжить». Синхронизация выбора озвучек включается мастером отдельно, а каждый участник может локально отключить её и оставить комнатную синхронизацию времени. При первом удалённом запуске браузер участника требует явный клик по кнопке запуска, кнопка показывается поверх плеера. Если участник досмотрел серию раньше мастера, он остаётся на закончившейся серии в паузе и ждёт команду мастера; локальный автопереход Kodik и локальное сохранённое «Продолжить» в комнате игнорируются, переход на следующую серию рассылает только мастер через команду совместного просмотра, а разрешение выбора серий действует только на ручной выбор через кнопки серий. Если мастер выходит, сервер передаёт роль следующему участнику. Админка показывает read-only список активных комнат, участников и текущий просмотр. Комнаты хранятся только в памяти WebSocket-процесса.
