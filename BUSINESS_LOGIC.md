...
6. Обычные страницы `/api/releases` и server feed cache живут 1 час с tag `releases`; polling `live=1` остаётся без кэша
7. Карточки фильмов и спешлов показывают короткий бейдж `M`/`S` на постере, а hover-панель показывает тип тайтла с описанием.
...
**Fullscreen TA-плеер:** прозрачная полоса под нижними контролами принимает клик для фокуса, но не переключает play/pause и не отменяет прокрутку.
