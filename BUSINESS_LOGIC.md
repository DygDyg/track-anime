...
11. Beta-плеер может создать WebSocket-комнату совместного просмотра, если глобальная настройка `WatchPartySettings.enabled` включена. В комнате есть invite-ссылка, список участников и мастер. Неавторизованные пользователи входят со случайным локальным именем вида «Гость 1234», если `WatchPartySettings.allowGuests` включена. Мастер управляет play/pause, seek и сериями; play/pause и seek/выбор серий для остальных включаются отдельными разрешениями мастера. Озвучка не синхронизируется принудительно: каждый участник выбирает её локально, список участников показывает текущую озвучку/серию, а позиция комнаты синхронизируется напрямую в секундах (`positionSeconds`) как seek API Kodik. При первом удалённом запуске браузер участника требует явный клик по кнопке запуска. Если мастер выходит, сервер передаёт роль следующему участнику. Комнаты хранятся только в памяти WebSocket-процесса.
