...
5. Обычные страницы `/api/releases` и server feed cache живут 1 час с tag `releases`; polling `live=1` остаётся без кэша
6. Карточки фильмов и спешлов показывают короткий бейдж `M`/`S` на постере, а hover-панель показывает тип тайтла с описанием.
...
- **Android APK:** вкладка «Приложение» показывает версию из `/downloads/TrackAnime.json` и ведёт на `/downloads/TrackAnime.apk`; оба файла появляются при деплое с параметром `-ApkPath`. Приложение предлагает обновление только при большем `versionCode`, сверяет SHA-256 и передаёт установку системному Android.
