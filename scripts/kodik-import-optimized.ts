...
async function importCatalogPhase() {
  ...
  while (nextUrl) {
    ...
    totalProcessed += pageSaved;
    const elapsed = Math.floor((Date.now() - startTime) / 1000);
    
    console.log(
      `[catalog] +${pageSaved} | всего ${totalProcessed}/${page.total} | время: ${elapsed}s`,
    );
...
async function importEpisodesPhase() {
  ...
  // Используем оптимизированный импорт серий
  console.log(`=== Фаза 2: серии всех озвучек (осталось ${pending}) ===`);
  
  // Здесь будет оптимизированная логика импорта серий
...
// Функция для оптимизированного импорта серий
async function runKodikEpisodesImport() {
  // Замените здесь на реализацию оптимизированного импорта серий
  return {
    processed: 0,
    skipped: 0,
    newEpisodes: 0,
